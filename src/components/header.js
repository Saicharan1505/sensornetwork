...
const Header = () => {
  ...
  const handleNavigation = (path) => {
    if (path === '/') {
      // Setting a flag to indicate the user came from "Home" button
      sessionStorage.setItem('fromHome', 'true');
    }
    navigate(path);
    setIsNavOpen(false); // Close navbar after selection
  };
      ...
      {isNavOpen && (
        <nav style={{
          ...
          {[
            ...
            <button key={index} onClick={() => handleNavigation(item.path)} style={{
              background: 'none',
              border: 'none',
              color: 'white',
              textAlign: 'left',
